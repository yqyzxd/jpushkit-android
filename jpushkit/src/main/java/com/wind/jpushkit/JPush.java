...
public class JPush {
     ...
     * @return
     */
    private static boolean fromJpush(Intent intent) {
        if (intent == null) {
            return false;
        }
        String data = null;
     ...
     * @return
     */
    private static boolean fromNetease(Intent intent) {
        if (intent == null) {
            return false;
        }
        String data = null;
    ...
    public static void handleOpenClick(Intent intent, Context context) {
        ...
        try {
            if (fromManufacturerChannelPush(intent)) {
                if (fromNetease(intent)) {
                    handleNeteaseClick(intent, context);
                } else if (fromJpush(intent)) {
                    handleJpushClick(intent, context);
                }
    ...
    private static void handleNeteaseClick(Intent intent, Context context) throws Exception {
        String sessionID = resolveIntent(intent);
        Class clazz = Class.forName("com.netease.nim.uikit.SessionHelper");
        //Context context, String account
        Method method = clazz.getMethod("startP2PSession", Context.class, String.class);
        method.invoke(null, context, sessionID);
    }

    private static void handleJpushClick(Intent intent, Context context) throws Exception {
